...
CamConnectionPanel::CamConnectionPanel(QWidget *parent)
    ...
    connect(statusLogWindow, &StatusLogWindow::logsCleared, this,
            &CamConnectionPanel::logsClearedSlot);
    ...
    QGridLayout *camconfigurecontainerlayout =
        new QGridLayout(camConfigureContainer);
    ...
    connect(camTypeCombo, &QComboBox::currentTextChanged, this,
            &CamConnectionPanel::typeComboChangedSlot);
    ...
    connect(infoToolBarLabel, &QLineEdit::editingFinished, this,
            &CamConnectionPanel::nameEditedSlot);
    ...
    connect(deleteAction, &QAction::triggered, this,
            &CamConnectionPanel::deleteCamClickedSlot);
    ...
    connect(camStatusButton, &QPushButton::clicked, this,
            &CamConnectionPanel::showLogClickedSlot);
    QWidget *camStatusSpacer = new QWidget(camInfoWidget);
    camStatusSpacer->setSizePolicy(QSizePolicy::Expanding,
                                   QSizePolicy::Expanding);
    ...
    camconfigurelayout->addWidget(camConfigInterface->configEditBox, i++, 0, 1,
                                  2);
...
void CamConnectionPanel::addClickedSlot() { stackWidget->setCurrentIndex(1); }
...
void CamConnectionPanel::confirmConfigClickedSlot() {
    ...
    if (!streaming) {
        ...
        if (camNameEdit->text() == "")
            cameraName =
                "Camera_" + QString::number(CameraInterface::cameraList.size());
        else
            cameraName = camNameEdit->text();
        if (camConfigInterface->confirmConfigClicked()) {
            camera = camConfigInterface->getCamera(cameraName);
            connect(camera, &CameraInterface::statusUpdated, this,
            ...
            errorMessage.showMessage("No camera selected! Make sure there is a "
                                     "camera plugged in and "
...
void CamConnectionPanel::deleteCamClickedSlot() {
    if (!camera->isStreaming()) {
        ...
        camera = nullptr;
        stackWidget->setCurrentIndex(0);
        camStatusInfoIcon->setPixmap(
            statusIcons[Connecting].pixmap(QSize(20, 20)));
        camStatusInfo->setText(statusTexts[Connecting]);
        emit camListChanged();
    }
}
...
void CamConnectionPanel::typeComboChangedSlot(QString type) {
    QLayoutItem *item = camconfigurelayout->takeAt(
        camconfigurelayout->indexOf(camConfigInterface->configEditBox));
    delete item->widget();
    delete item;
    QLayoutItem *infoItem = caminfolayout->takeAt(
        caminfolayout->indexOf(camConfigInterface->configInfoBox));
    ...
    camconfigurelayout->addWidget(camConfigInterface->configEditBox, 2, 0, 1,
                                  2);
