...
TriggerConnectionPanel::TriggerConnectionPanel(QWidget *parent)
    ...
    connect(statusLogWindow, &StatusLogWindow::logsCleared, this,
            &TriggerConnectionPanel::logsClearedSlot);
    ...
    connect(addButton, &QPushButton::clicked, this,
            &TriggerConnectionPanel::addClickedSlot);
    ...
    connect(confirmAction, &QAction::triggered, this,
            &TriggerConnectionPanel::confirmConfigClickedSlot);
    exitButton = new QToolButton(this);
    exitAction = new QAction(this);
    createToolBarButton(exitButton, exitAction, QIcon::fromTheme("discard"),
                        true, false, QSize(35, 35));
    connect(exitAction, &QAction::triggered, this,
            &TriggerConnectionPanel::exitConfigClickedSlot);
    ...
    QLabel *triggerTypeLabel =
        new QLabel("Trigger Type", triggerConfigureWidget);
    ...
    connect(triggerTypeCombo, &QComboBox::currentTextChanged, this,
            &TriggerConnectionPanel::typeComboChangedSlot);
    ...
    connect(deleteAction, &QAction::triggered, this,
            &TriggerConnectionPanel::deleteTriggerClickedSlot);
    ...
    connect(triggerStatusButton, &QPushButton::clicked, this,
            &TriggerConnectionPanel::showLogClickedSlot);
    QWidget *triggerStatusSpacer = new QWidget(triggerInfoWidget);
    triggerStatusSpacer->setSizePolicy(QSizePolicy::Expanding,
                                       QSizePolicy::Preferred);
    triggerStatusInfoIcon->setPixmap(
    ...
    triggerconfigurecontainerlayout->setSpacing(0);
    i = 0;
    triggerinfolayout->addWidget(triggerConfigInterface->configInfoBox, i++, 0,
                                 1, 5);
...
void TriggerConnectionPanel::confirmConfigClickedSlot() {
    if (triggerConfigInterface->confirmConfigClicked()) {
        TriggerInterface::triggerInstance =
            triggerConfigInterface->getTrigger();
        connect(TriggerInterface::triggerInstance,
                &TriggerInterface::statusUpdated, this,
                &TriggerConnectionPanel::statusUpdatedSlot);
    } else {
        QErrorMessage errorMessage;
        errorMessage.showMessage("No trigger device selected! Make sure your "
...
void TriggerConnectionPanel::typeComboChangedSlot(QString type) {
    QLayoutItem *item = triggerconfigurelayout->takeAt(
        triggerconfigurelayout->indexOf(triggerConfigInterface->configEditBox));
    QLayoutItem *spacerItem = triggerconfigurelayout->takeAt(
        triggerconfigurelayout->indexOf(configureBottomSpacer));
    delete item->widget();
    delete item;
    QLayoutItem *infoItem = triggerinfolayout->takeAt(
        triggerinfolayout->indexOf(triggerConfigInterface->configInfoBox));
    ...
    triggerconfigurelayout->addWidget(triggerConfigInterface->configEditBox, 2,
                                      0, 1, 2);
    triggerconfigurelayout->addItem(spacerItem, 3, 0, 1, 2);
    triggerinfolayout->addWidget(triggerConfigInterface->configInfoBox, 0, 0, 1,
                                 5);
}
