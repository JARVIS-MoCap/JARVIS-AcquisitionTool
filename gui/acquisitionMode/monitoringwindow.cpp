...
MonitoringWindow::MonitoringWindow(QWidget *parent)
    ...
    connect(showGraphsAction, &QAction::toggled, this,
            &MonitoringWindow::showGraphsSlot);
...
void MonitoringWindow::updateListSlot() {
    ...
    for (const auto &cam : CameraInterface::cameraList) {
        ...
        connect(cam, &CameraInterface::latencyAndFrameNumberUpdate, newMonitor,
                &CameraMonitor::latencyAndFrameNumberUpdateSlot);
    }
    updateBufferStates();
}
