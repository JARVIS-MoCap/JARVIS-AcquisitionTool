...
SetupAllCamerasWindow::SetupAllCamerasWindow(QWidget *parent)
    ...
    settingsBox->setStyleSheet("QGroupBox{background-color:rgb(34, 36, 40)}");
    ...
    LabelWithToolTip *loadDefaultLabel =
        new LabelWithToolTip("  Load Defaults before Setup");
    ...
    LabelWithToolTip *externalTriggerLabel =
        new LabelWithToolTip("  Setup for External Trigger");
    ...
    LabelWithToolTip *exposureAutoLabel =
        new LabelWithToolTip("  Exposure Auto");
    ...
    connect(exposureAutoToggle, &ToggleSwitch::toggled, this,
            &SetupAllCamerasWindow::exposureAutoToggledSlot);
    LabelWithToolTip *exposureValueLabel =
        new LabelWithToolTip("  Exposure Time [ms]");
    ...
    connect(gainAutoToggle, &ToggleSwitch::toggled, this,
            &SetupAllCamerasWindow::gainAutoToggledSlot);
    ...
    connect(setupButton, &QPushButton::clicked, this,
            &SetupAllCamerasWindow::setupButtonClickedSlot);
...
void SetupAllCamerasWindow::loadSettings() {
    ...
    if (settings->contains("LoadDefaults")) {
        m_cameraSettings.loadDefaults =
            settings->value("LoadDefaults").toBool();
    }
    if (settings->contains("UseExternalTrigger")) {
        m_cameraSettings.useExternalTrigger =
            settings->value("UseExternalTrigger").toBool();
    }
    if (settings->contains("ExposureAuto")) {
        m_cameraSettings.exposureAuto =
            settings->value("ExposureAuto").toBool();
    }
    if (settings->contains("ExposureTime")) {
        m_cameraSettings.exposureTime =
            settings->value("ExposureTime").toFloat();
    }
...
void SetupAllCamerasWindow::saveSettings() {
    ...
    settings->setValue("UseExternalTrigger",
                       m_cameraSettings.useExternalTrigger);
