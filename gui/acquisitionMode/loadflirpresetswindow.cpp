...
LoadFlirPresetsWindow::LoadFlirPresetsWindow(FLIRChameleon *cam,
                                             QWidget *parent)
    : LoadCameraPresetsInterface(parent), m_cam(cam) {
    ...
    connect(userSetList, &QListWidget::currentItemChanged, this,
            &LoadFlirPresetsWindow::currentItemChangedSlot);
    usersetgrouplayout->addWidget(userSetList, 0, 0);

    QLabel *defaultUserSetLabel =
        new QLabel("Default User Set (loaded on powerup):  ");
    ...
    connect(defaultUserSetCombo, &QComboBox::currentTextChanged, this,
            &LoadFlirPresetsWindow::defaultUserSetChangedSlot);
