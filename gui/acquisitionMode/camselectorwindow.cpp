...
CamSelectorWindow::CamSelectorWindow(QWidget *parent)
    ...
    camsTable->horizontalHeader()->setSectionResizeMode(0,
                                                        QHeaderView::Stretch);
    camsTable->verticalHeader()->hide();
    camsTable->setShowGrid(false);
    camsTable->setSelectionBehavior(QAbstractItemView::SelectRows);

    connect(camsTable, &QTableWidget::cellDoubleClicked, this,
            &CamSelectorWindow::selectionChangedSlot);
    connect(camsTable, &QTableWidget::itemPressed, this,
            &CamSelectorWindow::tableItemPressed);
    connect(camsTable, &QTableWidget::itemClicked, this,
            &CamSelectorWindow::tableItemClicked);
...
void CamSelectorWindow::selectionChangedSlot(int row, int) {
    for (int i = 0; i < camsTable->rowCount(); i++) {
        if (i == row)
            camsTable->item(i, 0)->setFont(
                QFont("Sans Serif", 11, QFont::Bold));
        else
            camsTable->item(i, 0)->setFont(QFont("Sans Serif", 11));
    }
...
void CamSelectorWindow::statusUpdatedSlot(CameraInterface *cam,
    ...
    camsTable->item(CameraInterface::cameraList.indexOf(cam), 0)
