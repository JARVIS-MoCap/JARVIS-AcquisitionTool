...
PresetsWindow::PresetsWindow(QList<QString> *presets, const QString &type,
    ...
    *m_presets = settings->value(m_name + "/Presets").value<QList<QString>>();
    ...
    createToolBarButton(discardButton, discardAction,
                        QIcon::fromTheme("discard"), false, false,
                        QSize(40, 40));
    connect(discardAction, &QAction::triggered, this,
            &PresetsWindow::discardClickedSlot);
    ...
    connect(newPresetEdit, &QLineEdit::textEdited, this,
            &PresetsWindow::presetNameEditedSlot);
    connect(newPresetEdit, &QLineEdit::returnPressed, this,
            &PresetsWindow::saveClickedSlot);
    ...
    connect(presetsList, &QListWidget::currentRowChanged, this,
            &PresetsWindow::currentPresetChangedSlot);
    connect(presetsList, &QListWidget::itemClicked, this,
            &PresetsWindow::presetClickedSlot);
    connect(presetsList, &QListWidget::currentItemChanged, this,
            &PresetsWindow::currentItemChangedSlot);
...
void PresetsWindow::loadClickedSlot() {
    emit loadPreset(m_name + "/" +
                    m_presets->value(presetsList->currentRow() / 2));
    close();
}
...
void PresetsWindow::saveClickedSlot() {
    ...
    if (newPresetEdit->text() != "" &&
        m_presets->indexOf(newPresetEdit->text()) == -1) {
