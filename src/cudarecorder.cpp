...
CudaRecorder::CudaRecorder(const QString& cameraName, const AcquisitionSpecs& acquisitionSpecs)
	...
	m_cudaJPEGEncoder = new CudaJPEGEncoder(acquisitionSpecs.frameSize.width, acquisitionSpecs.frameSize.height);
}
...
QPixmap CudaRecorder::recordFrame(uchar * frame) {
	...
	QImage img = QImage(img_data, m_acquisitionSpecs.frameSize.width, m_acquisitionSpecs.frameSize.height, QImage::Format_RGB888);
	QPixmap pix = QPixmap();
