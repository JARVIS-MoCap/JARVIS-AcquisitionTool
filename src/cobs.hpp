// Source: https://github.com/PatrickBaus/COBS-CPP
/**
 ...
 *
 * @param buffer The i/o buffer. The data must start at index 1. Index 0 will be
 * overwritten with the overhead byte
 * @param size of the buffer to be encoded. Must be at least
 * two and not greater than 255 The size of the data block must be at most 254
...
static size_t encode(uint8_t *buffer, const size_t size) {
    ...
    // This 0x00 byte will be overwritten later, but serves as a terminator for
    // the parser for now.
    buffer[0] = 0x00;
    ...
    do {
        ...
        // data block. It serves as a terminator and saves us a bounds check
        // while iterating the loop.
        ...
        *cursor = endOfBlock - cursor +
                  1; // Calculate the number of bytes until the next 0x00 byte
...
/**
 * Decodes a COBS (Consistent Overhead Byte Stuffing) buffer in place. The
 *encoded data will be *overwritten*!
 *
 * @param *buffer A pointer to the buffer containing the encoded data, without
 *the delimiter/framing byte. After decoding the buffer will contain the data
 *without the overhead byte. The overhead byte is the first byte, so the data
 *stream will have an offset of +1.
...
static size_t decode(uint8_t *buffer, const size_t size) {
    ...
    // 254 bytes (unencoded). Therefore the maximum input size is 255 encoded
    // bytes.
    ...
    do {
        ...
        *buffer = 0x00;
        buffer +=
            tmp; // If we are out of bounds, this will be the last iteration
