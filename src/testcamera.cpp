...
TestAcquisitionWorker::TestAcquisitionWorker(const QString& cameraName,
...
TestCamera::TestCamera(QString cameraName, const QString& basepath,
	const QString&) : CameraInterface(testCamera, cameraName) {
	createSettings();
...
void TestCamera::startAcquisitionSlot(AcquisitionSpecs acquisitionSpecs) {
	if (!m_isStreaming) {
		...
		m_acquisitionWorker = new TestAcquisitionWorker(m_cameraName,
					acquisitionSpecs, testimgs);
		m_acquisitionWorker->moveToThread(&workerThread);
		connect(this, &TestCamera::startAcquisition,
						m_acquisitionWorker, &AcquisitionWorker::acquireImages);
		connect(m_acquisitionWorker, &AcquisitionWorker::streamImage,
						this, &TestCamera::streamImageSlot);
...
void TestCamera::createSettings() {
	...
	CategoryNode *analogControlNode = new CategoryNode(m_cameraSettingsRootNode,
				"AnalogControl");
	...
	CategoryNode *deviceControlNode = new CategoryNode(m_cameraSettingsRootNode,
				"DeviceControl");
