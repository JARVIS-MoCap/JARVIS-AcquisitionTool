    ...
    if (m_acquisitionSpecs.record) {
        ...
        if (true) {
            ...
            m_videoWriter = new cv::VideoWriter(
                outvideoName_str, cv::VideoWriter::fourcc('M', 'J', 'P', 'G'),
...
QImage BaseRecoder::recordFrame(uchar *frame) {
    cv::Mat img_bayer(m_acquisitionSpecs.frameSize.height,
                      m_acquisitionSpecs.frameSize.width, CV_8UC1, frame,
                      cv::Mat::AUTO_STEP);
    cv::Mat img_cv, img_stream;
    cv::cvtColor(img_bayer, img_cv, cv::COLOR_BayerBG2BGR);
    cv::resize(img_cv, img_stream,
               cv::Size(m_acquisitionSpecs.frameSize.width /
    ...
    QImage img = QImage(img_stream.data,
